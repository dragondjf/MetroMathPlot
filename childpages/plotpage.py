    ...
    def ShowHandler(self):
        for index in range(self.fvbox.count() - 1):
            ...
            if not plotfig.isVisible():
                ...
                if plotfig.plotmode == "by data":
                    if hasattr(plotfig, "datahandler"):
                        if plotfig.datahandler.isAlive():
                            plotfig.datahandler.event.clear()
                elif plotfig.plotmode == "by timer":
                    if not hasattr(plotfig, 'timer'):
                        plotfig.timer = plotfig.startTimer(timeinteral)

                getattr(plotfig, 'StartButton').setEnabled(True)
                getattr(plotfig, 'PauseButton').setEnabled(False)
...
class PlotWidget(QtGui.QWidget):
    ...
    """
    def __init__(self, parent=None, point_num=300):
...
class PlotFigure(PlotWidget):
    def __init__(self, point_num=300):
        super(PlotFigure, self).__init__(point_num=point_num)
        ...
        self.plotmodes = ["by data", "by timer"]
        self.plotmode = "by data"
    ...
    def hideHandler(self):
        ...
        if self.plotmode == "by data":
            if hasattr(self, "datahandler"):
                if self.datahandler.isAlive():
                    self.datahandler.event.clear()
        elif self.plotmode == "by timer":
    ...
    def settings(self, kargs):
        ...
        if 'wavpath' in kargs:
            ...
            if not hasattr(self, 'datahandler'):
                self.datahandler = WaveReplayHandler(self.objectName(), self.wavfiles, self.settintparameter['importwavspreed'], self.plotmode)
                self.datahandler.plotsignal.connect(self.refresh)
        ...
        if isinstance(self.datahandler, WaveReplayHandler):
            self.datahandler.importspreed = self.settintparameter['importwavspreed']
        getattr(self, 'PauseButton').setEnabled(True)
        getattr(self, 'StartButton').setEnabled(False)
